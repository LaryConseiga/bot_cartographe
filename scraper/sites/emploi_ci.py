"""
Source : emploi.ci (Côte d’Ivoire).
Stratégie : HTML statique BeautifulSoup, pagination défensive, description complète
si extrait carte < 100 caractères.
"""
...
def _extract_cards(soup, list_url: str) -> list[tuple[str, str, str, str]]:
    ...
    for c in cards:
        ...
        company_el = c.select_one(".company, .employer, .recruteur")
        loc_el = c.select_one(".location, .lieu, .ville")
    ...
    if not out:
        for a in soup.select("a[href*='offre'], a[href*='emploi'], a[href*='job']"):
            url = make_absolute(a.get("href", ""), list_url)
